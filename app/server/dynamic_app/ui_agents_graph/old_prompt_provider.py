...
def get_ui_prompt(base_url: str, examples: str, custom_schemas: list = None) -> str:
    """
    ...
    Args:
        ...
        custom_schemas: A list of custom component schemas to inject into the A2UI schema.
    ...
    schema_str = A2UI_SCHEMA

    # Inject custom component schemas if provided
    if custom_schemas:
        try:
            schema_obj = json.loads(schema_str)

            # Add custom components to the component properties
            ...
            # If there's an error, fall back to the base schema
            print(f"Warning: Failed to inject custom schemas: {e}")
    ...
    -   If the number of restaurants is 5 or fewer, you MUST use the `SINGLE_COLUMN_LIST_EXAMPLE` template (shows restaurants as individual cards with images and details).
    -   If the number of restaurants is more than 5, you MUST use the `TWO_COLUMN_LIST_EXAMPLE` template (shows restaurants in a 2-column grid layout).
    -   If the query is to book a restaurant (e.g., "USER_WANTS_TO_BOOK..."), you MUST use the `BOOKING_FORM_EXAMPLE` template (shows a form for making reservations).
    -   If the query is a booking submission (e.g., "User submitted a booking..."), you MUST use the `CONFIRMATION_EXAMPLE` template (shows booking confirmation details).
    -   If the query is about comparing restaurants or showing a restaurant chart/table (e.g., "compare restaurants", "show me a restaurant chart", "restaurant ratings comparison"), you MUST use the `RESTAURANT_TABLE_EXAMPLE` template (shows a bar chart visualization of restaurant ratings).
...
if __name__ == "__main__":
    ...
    # Define custom component schemas
    custom_schemas = [
        {
            "name": "RestaurantBarChart",
            "schema": {
                "type": "object",
                "properties": {
                    "restaurants": {
    ...
    restaurant_prompt = get_ui_prompt(my_base_url, RESTAURANT_UI_EXAMPLES, custom_schemas)
